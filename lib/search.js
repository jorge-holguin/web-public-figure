...
export async function getContextForQuery(query) {
  try {
    ...
    if (query === "comisiones completas" || /^comisiones completas$/i.test(query)) {
        const comisionesDoc = extraDocs.find(doc => doc.type === "comisiones");
        return comisionesDoc ? comisionesDoc.text : "No se encontró información sobre las comisiones.";
      }
    ...
    // 7.0 Caso: la consulta parece ser sobre comisiones
    if (/comision(es)?|comit(e|é)|grupo|miembro de/i.test(query)) {
      console.log("La consulta parece ser sobre comisiones");
      
      // Buscar el documento de comisiones en los documentos extra
      const comisionesDoc = documents.find(doc => doc.type === "comisiones");
      
      if (comisionesDoc) {
        // Si la consulta es sobre el listado completo de comisiones
        if (/listad?o?|todo|completo|general|todos|ver/i.test(query)) {
          console.log("Mostrando listado completo de comisiones");
          return comisionesDoc.text;
        }
        
        // Si hay filtros específicos para comisiones (por ejemplo, por año)
        if (yearFilter) {
          // Podríamos filtrar las comisiones por año si el documento tiene esa estructura
          // Esto dependerá de cómo estén estructurados los datos de comisiones
          console.log(`Buscando comisiones del año ${yearFilter}`);
          
          // Ejemplo: Si el texto contiene secciones por año que podemos extraer
          const yearPattern = new RegExp(`Comisiones ${yearFilter}[\\s\\S]*?(?=\\n\\nComisiones \\d{4}|$)`, 'i');
          const yearMatch = comisionesDoc.text.match(yearPattern);
          
          if (yearMatch) {
            return yearMatch[0];
          } else {
            return `No se encontró información sobre comisiones para el año ${yearFilter}.`;
          }
        }
        
        // Búsqueda general dentro del texto de comisiones
        // Usamos una búsqueda de texto simple para encontrar coincidencias dentro del documento
        const searchTerms = query.replace(/comision(es)?|comit(e|é)|grupo|miembro de/gi, '').trim();
        
        if (searchTerms.length > 0) {
          console.log(`Buscando términos específicos en comisiones: "${searchTerms}"`);
          ...
          // Recopilar todas las oraciones que contienen el término de búsqueda
          while ((match = searchRegex.exec(comisionesDoc.text)) !== null) {
            matches.push(match[1].trim());
          }
          
          if (matches.length > 0) {
            return `Información sobre comisiones relacionada con "${searchTerms}":\n\n${matches.join('\n\n')}`;
          }
        }
        
        // Si no hay términos específicos o no se encontraron coincidencias, devolver el documento completo
        return comisionesDoc.text;
      } else {
        return "No se encontró información sobre las comisiones en la base de datos.";
      }
    }
    ...
    if (results.length === 0) {
      ...
      // 10.4. Si no hay resultados y la consulta es sobre comisiones
      if (/comision(es)?|comit(e|é)|grupo|miembro de/i.test(query)) {
        const comisionesDoc = documents.find(doc => doc.type === "comisiones");
        if (comisionesDoc) {
          console.log("No hay coincidencias exactas. Mostrando información de comisiones completa.");
          return "Aquí tienes la información sobre las comisiones del congresista Héctor Valer:\n\n" + comisionesDoc.text;
        }
      }

      // 10.5. Si no se encontraron resultados, mensaje genérico
